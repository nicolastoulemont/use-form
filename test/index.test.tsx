...
function BasicComponent() {
  ...
  const initialFields: Array<Field> = [
    ...
    },
    {
      name: 'add',
      listener: {
        onChange: [() => addFields([{ name: 'new_field' }], initialFields.length)],
      },
    },
    {
      name: 'add-multiple',
      listener: {
        onChange: [
          () => addFields([{ name: 'new_field_1' }, { name: 'new_field_2' }], initialFields.length),
        ],
      },
    },
    {
    ...
    <div>
      {fields.map((field, index) => (
        <div key={index}>
          ...
          />
          {errors[field.name] && (
            <div data-testid={field.name.concat('-error')}>{errors[field.name]}</div>
          )}
        </div>
      ))}
      ...
      <button
        data-testid="set-val"
        onClick={() => setValues({ email: 'settest@test.com', password: 'setpassword' })}
      >
        set Values
      </button>
      <button
        data-testid="set-err"
        onClick={() => setErrors({ email: 'email error', password: 'password error' })}
      >
        set Error
      </button>
    </div>
